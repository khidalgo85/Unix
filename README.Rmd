...
> 🇪🇸 **Nota**
> 
> * Cuando la caja **NO** não é um comando e sim uma saída, ósea lo que ud debería en su pantalla, después de rodar el comando. Será avisado como * es decir lo que ud deberia ver en su pantalla después de correr el comando **output**.
...
> 🇪🇸
> 
> Ahora entre en la carpeta 📁 `analises/` y cree otra 📁 llamada `coleta_2020`, confirme con `ls`
...
> 🇪🇸
> 
> Ahora entre a la carpeta que acabó de crear usando el comando `cd` y verifique donde está con `pwd`, después regrese para el directorio base `root/` 📁 y verifique nuevamente.
>
...
> 🇪🇸
> 
> Ahora, usted quiere entrar en esa pasta nueva. Use el comando `cd` para cambiar de carpeta y use [Tab] [Tab] para que el sistema muestre las opciones con comienzo "col".
...
> 🇪🇸
> 
> Ud usó `../`, porque ud estaba dentro de la carpeta 📁 `/root/tutorial/` y necesitaba volver para 📁 `/root/` (📁 carpeta arriba de `tutorial/`) para continuar el camino para 📁 `dados/`.
>
...
Para renomear muitos arquivos *.old* a *.new. Para testar primeiro, susbtituia `do mv` por `do echo mv`. 
...
🇧🇷 Para a instalação de cada ferramenta, visite o site [https://anaconda.org/](https://anaconda.org/) e na caixa de busca procure pela ferramenta que deseja e encontrara o comando para instalação. 
